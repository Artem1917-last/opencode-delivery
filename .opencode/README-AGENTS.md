...
Agents are converted to `.md` files and placed in `.opencode/agents/`.
...
Each agent becomes a `.mdc` rule file in `.cursor/rules/`.
...
All agents are combined into a single `CONVENTIONS.md` file.
...
```
/read CONVENTIONS.md
Use the Frontend Developer conventions to review this component.
```
...
All agents are combined into a single `.windsurfrules` file.
...
Each agent gets `SOUL.md`, `AGENTS.md`, and `IDENTITY.md` files.
...
Agents are converted to `.md` SubAgent files in `~/.qwen/agents/`.

```bash
./scripts/install.sh --tool qwen
```

See [integrations/qwen/README.md](integrations/qwen/README.md) for details.
</details>

<details>
<summary><strong>Kimi Code</strong></summary>

Agents are converted to YAML specs in `~/.config/kimi/agents/`.
...
You are **Agent Name**, a [role description]. You specialize in [key areas].
...
1. **Personality First**: Start with who the agent is, not what it does
2. **Deliverable Focus**: Every agent should produce concrete outputs
3. **Workflow Driven**: Include step-by-step processes
4. **Success Metrics**: Define measurable outcomes
5. **Memory Patterns**: Include learning and pattern recognition

---

## 🌟 Success Stories

> "Used the Frontend Developer agent to build our entire React component library in 2 days. What would have taken 2 weeks."
> 
> -- Startup CTO

> "The Paid Media Auditor found $50k/month in wasted ad spend we'd been burning for 6 months."
> 
> -- Marketing Director

> "Our code review process improved 300% after implementing the Code Reviewer agent."
> 
> -- Engineering Manager
...
- **Issues**: [GitHub Issues](https://github.com/msitarzewski/agency-agents/issues)
- **Discussions**: [GitHub Discussions](https://github.com/msitarzewski/agency-agents/discussions)
- **Sponsor**: [GitHub Sponsors](https://github.com/sponsors/msitarzewski)
